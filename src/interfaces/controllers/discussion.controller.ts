...
export class DiscussionController {
  constructor(private discussionUseCase: DiscussionUseCase,private studentUseCase:StudentUseCase) {}

  async createDiscussion(req: Request, res: Response): Promise<void> {
    try {
      const orderId = req.params.id;
      const data = await this.discussionUseCase.createDiscussion(
        orderId,
        req.body.text
      );
      res
      ...
      res
        ...
        .json(
          errorResponse(error.message || FAILED_DISCUSSION_CREATE)
        );
    }
  }
  ...
  async getAllDiscussions(req: Request, res: Response): Promise<void> {
    try {
      const orderId = req.params.id;
      const discussions = await this.discussionUseCase.getAllDiscussions(
        orderId
      );
      res
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json(
          errorResponse(error.message || FAILED_DISCUSSIONS_FETCH)
        );
    }
  }
  ...
  async createReact(req: Request, res: Response): Promise<void> {
    try {
      ...
      const data = await this.discussionUseCase.createReact(
        req.params.id,
        type
      );
      res
  ...
  async createReplay(req: IAuthanticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const updatedDiscussion = await this.discussionUseCase.addReply(
        discussionId,
        reply
      );
      res
  ...
  async getReplay(req: Request, res: Response): Promise<void> {
    try {
      const discussionId = req.params.id;
      const replies = await this.discussionUseCase.getReplay(discussionId);
      res
        .status(StatusCodes.OK)
